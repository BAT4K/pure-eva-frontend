...
export function WhyChooseUs() {
  ...
  return (
    <section className="border-t border-[#B2EBF2] bg-[#FAFAF9] px-6 py-20 md:px-12 md:py-28 lg:px-20 lg:py-32 overflow-x-hidden">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="grid gap-8 md:grid-cols-3 md:gap-6 lg:gap-10">
          {reasons.map((reason, index) => (
            <div 
              key={reason.title}
              className="group relative overflow-hidden rounded-3xl bg-white border border-[#B2EBF2] p-8 shadow-sm transition-all hover:shadow-xl hover:shadow-[#A5D6A7]/15 lg:p-10"
            >
              {/* Decorative number */}
              <span className="absolute -right-4 -top-6 font-serif text-[120px] leading-none text-[#A5D6A7]/8">
                {index + 1}
              </span>

              {/* Icon */}
              <div className="relative mb-6 inline-flex h-16 w-16 items-center justify-center rounded-2xl bg-[#F0FFF4] border border-[#B2EBF2] shadow-sm transition-transform group-hover:-translate-y-1 group-hover:shadow-md">
                <reason.icon className="h-7 w-7 text-[#4DD0E1]" />
              </div>
