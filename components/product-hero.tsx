        ...
        <div className="flex flex-col lg:grid lg:grid-cols-2 lg:items-start gap-12 lg:gap-20 relative">
          ...
          <div className="order-1 lg:col-start-2 lg:row-start-1 flex flex-col pt-4 lg:pt-0">
              ...
              <h1 className="mb-4 font-serif text-4xl leading-tight text-[#212121] md:text-5xl lg:text-6xl">
                {title.split(/(Cleanser)/i).map((part, i) => 
                  part.toLowerCase() === 'cleanser' ? (
                    <span key={i} className="text-[#34D399]">{part}</span>
                  ) : (
                    part
                  )
                )}
            ...
            <ScrollReveal delay={0.3}>
              <div className="flex items-baseline gap-3">
                ...
                {compareAtPrice && (
                  <>
                    <span className="text-xl text-gray-700 line-through">₹{compareAtPrice}</span>
                    <span className="rounded-full bg-[#34D399]/10 px-2.5 py-0.5 text-xs font-semibold tracking-wide text-[#34D399]">
                      SALE
                    </span>
                  </>
                )}
          ...
          <ScrollReveal delay={0.2} className="order-2 lg:col-start-1 lg:row-start-1 lg:row-span-2 self-start w-full">
            <div className="relative aspect-[4/5] w-full overflow-hidden rounded-3xl bg-gradient-to-br from-[#E0F7FA] to-[#B2EBF2] shadow-[0_25px_60px_-15px_rgba(0,0,0,0.1)] group">
              <div className="absolute -left-6 -top-6 h-32 w-32 rounded-full bg-[#34D399]/10 blur-2xl" />
                ...
                {imageUrl ? (
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center p-8">
                    <div className="mb-4 rounded-full bg-white/60 p-6 shadow-lg backdrop-blur-sm">
                      <Droplets className="h-12 w-12 text-[#34D399]" />
                    </div>
                  </div>
                )}
              </div>
          ...
          <div className="order-3 lg:col-start-2 lg:row-start-2">
              ...
              <div className="mb-8 max-w-md mt-6 lg:mt-0">
                <div className="relative">
                  <motion.div 
                    ...
                    className={`leading-loose text-gray-800 [&>p]:mb-4 [&>ul]:mb-4 [&>ul]:list-disc [&>ul]:pl-5 ${isExpanded ? "overflow-y-auto pr-2 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-[#B2EBF2] [&::-webkit-scrollbar]:w-1.5" : "overflow-hidden"}`}
                ...
                <button 
                  ...
                  className="group mt-2 flex items-center gap-1 text-sm font-medium text-[#34D399] transition-colors hover:text-[#10B981]"
                >
                  {isExpanded ? (
            ...
            <ScrollReveal delay={0.4}>
              <div className="mb-8 flex flex-wrap gap-3">
                {["Vegan", "Cruelty-Free", "Paraben-Free"].map((feature) => (
                  <div 
                    key={feature}
                    className="flex items-center gap-1.5 rounded-full border border-[#B2EBF2]/40 bg-white px-3 py-1.5 text-xs font-medium text-gray-800 shadow-sm transition-all hover:border-[#34D399] hover:shadow-md"
                  >
            ...
            <ScrollReveal delay={0.5}>
              <button 
                ...
                className="group flex w-full items-center justify-center gap-3 rounded-full bg-[#34D399] px-8 py-4 text-sm font-medium uppercase tracking-wider text-white shadow-lg transition-all duration-500 hover:scale-[1.02] hover:bg-[#10B981] hover:shadow-2xl hover:shadow-[#34D399]/30 md:w-auto"
              >
                <ShoppingBag className="h-4 w-4 transition-transform group-hover:-translate-y-0.5" />
                Add to Cart
              </button>

              <div className="mt-8 flex items-center gap-6 text-xs font-medium text-gray-700">
              ...
              <div className="mt-8 border-t border-[#B2EBF2] pt-6">
                <button
                  ...
                  className="flex w-full items-center justify-between font-serif text-lg text-[#212121] hover:text-[#34D399] transition-colors"
                >
                ...
                >
                  ...
                  <div className="flex flex-wrap gap-2 pb-2">
                    {["Aqua", "Glycerin", "Cocamidopropyl Betaine", "Allantoin", "Citric Acid", "Aloe Barbadensis Leaf Extract", "Chamomilla Recutita Extract", "Panthenol", "Sodium Hyaluronate"].map((ingredient) => (
                      <span 
                        key={ingredient}
                        className="rounded-full border border-[#B2EBF2]/40 bg-white px-3 py-1.5 text-[11px] font-medium text-gray-800 shadow-sm transition-all hover:border-[#4DD0E1]"
                      >
                        {ingredient}
                      </span>
                    ))}
